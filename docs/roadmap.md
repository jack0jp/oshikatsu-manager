...
`docs/prd.md` v0.4 のMVPスコープを、実装可能な順序に並べたもの。
...
- [ ] GitHub Actions
  ...
  - [ ] 自動コードレビュー(別モデル。まずローカルで指摘の質を見てからCIに載せる)
...
**先に決める必要があるもの**
- `profiles.is_admin` による強制削除を、MVPのRLSに含めるかどうか。
  `docs/data-model.md` §2 は「例外: is_admin は参加者の有無にかかわらず強制削除できる」と書き、
  `docs/prd.md` 4.6 と `docs/permissions.md` の権限マトリクスには管理者の行がない。
  **ポリシーに書くならマトリクスにも行を足してテストする。書かないならdata-model.mdの例外を消す。**
  どちらでもよいが、片方だけ実装された状態がいちばん危ない。
...
| ファイル | 内容 | 出どころ |
|---|---|---|
| `common/permissions.ts` | user_id・オーナーID・参加登録有無から操作可否 | permissions.md の権限マトリクス |
...
| 論点 | いつ効くか | 対処 |
|---|---|---|
| `app/` に判断ロジックが漏れる | フェーズ3〜4 | フェーズ3で機械的な検出手段を検討する |
| MCPとWeb UIの挙動差 | フェーズ5 | 同一操作の比較テスト。`common/` 経由の確認 |
| 生成型の差分検証がCLIバージョンで落ちる | フェーズ0〜1 | バージョンを固定する |
| `is_admin` 強制削除の扱いがドキュメント間で不一致 | フェーズ1 | 着手前に決める(上記) |
| 自動レビューの指摘が多すぎて読まれなくなる | フェーズ0 | ローカルで質を見てからCIに載せる(prd 8.5) |

## ドキュメントの不整合(要修正)

- `docs/prd.md` 付録が参照しているファイル名が実体と合っていない
  (`推し活管理アプリ_データモデル設計.md` → `docs/data-model.md`、
  `ai-review-pipeline-plan.md` は未作成)
- `docs/data-model.md` §2 の `is_admin` 強制削除の例外が、
  `docs/prd.md` 4.6 および `docs/permissions.md` の権限マトリクスに反映されていない
