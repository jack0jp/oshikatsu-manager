...
| 自分の参加登録・取りやめ | ○ | ○ | ○ |
| 他人の参加登録を取りやめ | × | × | × |
| 他ユーザーの招待 | ○ (自分の参加登録は不要) | ○ | × (オーナーまたは参加登録が前提) |
| 自分のステータス変更 | ○ | ○ | — |
| 他人のステータス変更 | × | × | × |
| 自分の公開設定の変更 | ○ | ○ | — |
...
**招待できる条件: そのイベントのオーナー、またはそのイベントに参加登録しているユーザー。**
オーナーは**自分自身を参加登録していなくても招待できる**。`docs/data-model.md` 2章のとおり
`owner_id` は「情報の管理者」であって参加者ではなく、登録だけして自分は参加しないケースを
意図的にサポートしているため。ここで参加登録を必須にすると、招待するためだけに自分の
「自分のスケジュール」へ実態と異なる行を作らせることになる(しかも招待後にその行を消せば
同じ状態に到達できてしまうので、制限として機能しない)。
...
| オーナーでも参加者でもないユーザーが他人を招待 | 失敗 |
| 参加登録していないオーナーが他人を招待 | 成功 |
