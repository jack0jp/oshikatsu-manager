# 推し活管理アプリ データモデル設計 (v0.4)

前提: PostgreSQL (Supabase) / Supabase Auth (Google SSO) / RLSによる行レベル制御

## v0.4 での主な変更点

- 招待の条件を確定。**そのイベントのオーナーは、自分が参加登録していなくても招待できる**
  (2章の「`owner_id` は参加者ではない」という定義と、`docs/permissions.md` の
  「オーナーは招待○」を一致させるため)。RLSポリシーもこれに合わせて修正
...
> このため、他ユーザーの招待はオーナーであれば参加登録の有無を問わず実行できる
...
- 招待できるのは、**そのイベントのオーナー**、または**そのイベントに参加登録している
  任意のユーザー**(オーナー限定ではない)
  - オーナーは参加登録の有無を問わず招待できる。2章のとおり `owner_id` は参加者ではなく、
    登録だけして自分は参加しないケースを表現できる必要があるため
    (招待のためだけに自分の参加行を作らせると「自分のスケジュール」が実態と食い違う)
...
| event_participants | 本人の行 + 他ユーザーの `visibility = 'public'` の行 | 本人の行 + 招待による他ユーザーの行 ※2 ※3 | 本人の行のみ |
...
>
> ※3 招待できるのは、そのイベントに参加登録しているユーザー**または** `owner_id` が自分の
> ユーザー(3章「招待の仕様」)。オーナーは自分の参加行が無くても招待できる。
>
