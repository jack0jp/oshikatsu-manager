...
- **memory**: このリポジトリ固有の方針ではなく、エージェント自身の作業プロセス
  ...
  再利用できる場合。追加するたびにMEMORY.mdの行数を確認する(**200行を超えた分は
  読み込み時に切り捨てられる**ため、近づいていたら新規追加より先に既存エントリの
  統合・削除を検討する)
- **このskillまたは`CLAUDE.md`**: このリポジトリの運用ルールそのもの(PR手順、
  lint方針など)に関する教訓で、次にこのリポジトリで作業する人/エージェントが
  確実に踏襲すべき場合。軽微ならその場で更新する
- **別Issueを立てる**: 対応にまとまった実装・設定変更が必要で、今回のPRの範囲を
  超える場合。`docs/lint-policy.md`のdrain-then-ratchetと同じ考え方
