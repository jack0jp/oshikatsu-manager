...
- **サブエージェント**(既定): 上位モデルが下位モデルのサブエージェントを起動し、Sub-issueの
  番号を渡す。人間がモデルを切り替える手間がなく、計画に無い判断に当たったときの戻り先が
  そのまま呼び出し元(=計画を書いた層)になる
- **セッションごと切り替える**: 残りが長時間に及び、上位モデルを待機させる意味がない場合。
  人間が次のセッションを下位モデルで開く
